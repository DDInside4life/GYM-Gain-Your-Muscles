  ...
  return (
    <Card className="mt-6">
      <CardHeader>
        ...
        <Link href="/dashboard/exercises" className="text-xs font-semibold text-brand-500 dark:text-violet-300 inline-flex items-center gap-1 hover:underline">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {items.map((it, idx) => (
          <motion.div
            ...
            className="glass-card p-3 overflow-hidden group cursor-pointer hover-lift hover:shadow-glow-brand dark:hover:shadow-glow transition-shadow duration-300"
          >
            <div className={`relative h-36 rounded-xl bg-gradient-to-br ${it.gradient ?? "from-violet-500/20 via-brand-500/10 to-transparent"} grid place-items-center overflow-hidden border border-[var(--border)]`}>
              <div className="absolute inset-0 grid-bg opacity-20" />
              <div className="h-16 w-16 rounded-full bg-white/40 dark:bg-white/[0.06] border border-white/40 dark:border-white/10 grid place-items-center group-hover:scale-110 transition-transform duration-300 backdrop-blur-sm">
                <Dumbbell className="h-8 w-8 opacity-70" />
              </div>
            ...
            <div className="pt-3 px-1 pb-1">
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                ...
                <span className="text-[10px] px-2 py-0.5 rounded-full bg-violet-500/10 text-violet-500 dark:text-violet-300 font-medium">{it.reps}</span>
              </div>
            </div>
          </motion.div>
        ))}
