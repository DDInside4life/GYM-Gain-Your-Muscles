...
def build_test_prescription(
    ...
    return Prescription(
        ...
        is_test_set=True,
        test_instruction=(
            "Сделайте 2–3 разминочных подхода, затем 1 рабочий сет с запасом 2 "
            "повтора (RIR 2). Без отказа, техника строго чистая. Вес и повторы "
            "используются для пересчёта рабочих недель."
        ),
